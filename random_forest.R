...
#return ("La précision est : ", precision = conf_mat_2[1, 1] / sum(conf_mat_2[,1]))
      
# Recall (TP / (TP + FN))
      
#return ("La sensibilité est : ", recal = conf_mat_2[1, 1] / sum(conf_mat_2[1,]))
      
# F-score (TN / (TN + FP))
      
#return ("Le F1-score est : ", f1_score = 2* ((precision*recal) / (precision+recal)))
      
# Accuracy ( (TP + TN) / (TP + FP + TN + FN) )
      
#return("l'accuracy totale : ", accuracy = (conf_mat_2[1,1]+conf_mat_2[2,2]) / (conf_mat_2[1,1]+conf_mat_2[2,1]+
